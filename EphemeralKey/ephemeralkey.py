...
def main():
    ...
    cw, transactionid = cloud.do_device_tokenauthEK_requestcw(credentials.cloudflaretokens, iotaccesstoken, tid)
    
    rw, ek = token.do_token_auth_ek(intToList(int(cw,16)))
    
    transactionidresponse = cloud.do_device_tokenauthEK_replyrw(credentials.cloudflaretokens, iotaccesstoken, tid, cw, rw, transactionid)
    
    authenticationresult, claimid, ekresult = cloud.do_device_tokenauthEK_checkstatus(credentials.cloudflaretokens, iotaccesstoken, transactionid)
